...
class LoginAPI(APIView):
    ...
    def post(self, request):
        ...
        query1 = "select * from tenant_details.tbl_user where county_name = '"+username.lower()+"'"
        cursor.execute(query1)
        record = cursor.fetchone()
        print("1st Select query :  ",record)
        Insertedpassword = DecodePassword(bytes(record[3], 'utf-8'))
        if Insertedpassword == password:
            query1 = "select * from tbl_county_details where unique_id = '" + record[0] + "'"
            cursor.execute(query1)
            record2 = cursor.fetchone()
            conn.commit()
            conn.close()
            if len(record2)==5:
                return Response({"username":username,"unique_id":record2[0], "iframe":record2[1], "group_id":record2[2], "report_id":record2[3],"dataset_id":record2[4]},status=status.HTTP_200_OK)
            else:
...
def insert_data_into_rows(groupId, datasetId, postgresTableName, datasourceTableName, APPLICATION_ID,
    ...
    if len(rowsB) > 10000:
        last = 10000
        # bigTableDataLoad(0, last, v, groupId, datasetId, datasourceTableName, len(rowsB), rowsB)
        # return
    else:
        last = len(rowsB)
...
def cloneReport(groupId, groupIdFromClone, APPLICATION_ID, APPLICATION_SECRET, USER_ID, USER_PASSWORD):
    ...
    user_id = USER_ID
    user_password = USER_PASSWORD
    reportId = "d68d4c52-0c47-4cfe-ae12-f0a362f7b925"
    # # endpoint = "https://api.powerbi.com/v1.0/myorg/groups/" + groupId + "/datasets/" + datasetId + "/tables/" + tableName + "/rows"
    # endpoint = "https://api.powerbi.com/v1.0/myorg/groups/" + groupId + "/reports/ffb86feb-5e1a-4f59-9f97-21b53249bd63/Clone"
    # endpoint = "https://api.powerbi.com/v1.0/myorg/groups/"+groupIdFromClone+"/reports/"00e4196a-de76-4b29-887e-bed6b0e7a64d"/Clone"
...
def InsertIntoDataBaseCountyDetails(records, username, encPassword, countyName, uniqueId):
    ...
    schemaSet = "set schema 'tenant_details'"
    cursor.execute(schemaSet)
    query1 = '''  INSERT INTO tenant_details.tbl_user(
	county_unique_id, username, county_name, password)
	VALUES (%s, %s, %s, %s);'''
    record_to_insert1 = (uniqueId, username, countyName, encPassword)
    cursor.execute(query1, record_to_insert1)
    query2 = ''' INSERT INTO tenant_details.tbl_county_details(
	unique_id, iframe, group_id, report_id, dataset_id)
	VALUES (%s, %s, %s, %s,%s) '''
    record_to_insert2 = (uniqueId, records[3], records[0], records[1], records[2])
...
def signUp(username, countyName, password):
    ...
    time.sleep(900)
    records = PowerBiAutoMationAPI(countyName)
